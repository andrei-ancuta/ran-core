...
public class Login {
        ...
        try {
            for (i = 0; i < callbacks.length; i++) {
                if (callbacks[i] instanceof PagePropertiesCallback) {
                    handlePagePropertiesCallback((PagePropertiesCallback) callbacks[i]);
    ...
    private void handlePagePropertiesCallback(PagePropertiesCallback ppc) {
        System.out.println(ppc.getHeader());
    }
...
//            only for OpenAM12
    ...
    protected boolean login(AuthContext lc)
            throws UnsupportedCallbackException, L10NMessageImpl {
        boolean succeed = false;
        Callback[] callbacks = null;
        
        // get information requested from module
        while (lc.hasMoreRequirements(true)) {

            callbacks = lc.getRequirements(true);
            if (callbacks != null) {
        ...
        if (lc.getStatus() == AuthContext.Status.SUCCESS) {
            ...
            System.out.println("token:" + lc.getSSOToken().getTokenID());

            succeed = true;
        } else if (lc.getStatus() == AuthContext.Status.FAILED) {
            System.out.println("Login failed.");
            System.out.println("Error code:" + lc.getErrorCode());
            System.out.println("Error message:" + lc.getErrorMessage());
            System.out.println("Error template: " + lc.getErrorTemplate());
        } else {
