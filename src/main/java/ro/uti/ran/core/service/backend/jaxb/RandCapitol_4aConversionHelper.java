 ...
 */
public class RandCapitol_4aConversionHelper {
     ...
     * @return jaxb pojo
     */
    public static SectiuneCapitol_4a toSchemaType(Cultura cultura) {
        if (cultura == null) {
            throw new IllegalArgumentException("Cultura nedefinit!");
        }
        SectiuneCapitol_4a sectiuneCapitol_4A = new SectiuneCapitol_4a();
        /*codNomenclator + codRand + denumire*/
        sectiuneCapitol_4A.setCodNomenclator(cultura.getCapCultura().getCod());
        sectiuneCapitol_4A.setCodRand(cultura.getCapCultura().getCodRand());
        sectiuneCapitol_4A.setDenumire(cultura.getCapCultura().getDenumire());
        /*nrHA + nrARI*/
        if (cultura.getSuprafata() != null) {
            ...
            if (tmp.get(SuprafataHelper.KEY_HA) != null) {
                ...
                sectiuneCapitol_4A.setNrHA(intreg_pozitiv);
            } else {
            ...
            if (tmp.get(SuprafataHelper.KEY_ARI) != null) {
                ...
                sectiuneCapitol_4A.setNrARI(fractie_doua_zecimale);
            } else {
        ...
        if (cultura.getGeometrieCulturi() != null) {
            ...
            sectiuneCapitol_4A.setGeometrieCultura(geometrie);
        }
        return sectiuneCapitol_4A;
    }
     ...
     *
     * @param sectiuneCapitol_4A jaxb pojo
     * @param gospodarie         entity pojo
     */
    public static void populeazaFromSchemaType(SectiuneCapitol_4a sectiuneCapitol_4A, Gospodarie gospodarie, Integer anRaportare) throws RequestValidationException {
        if (sectiuneCapitol_4A == null) {

            throw new RequestValidationException(RequestCodes.MISSING_ELEMENT, "sectiuneCapitol_4A");
        }
        if (gospodarie == null) {
        ...
        capCultura.setCod(sectiuneCapitol_4A.getCodNomenclator());
        capCultura.setCodRand(sectiuneCapitol_4A.getCodRand());
        capCultura.setDenumire(sectiuneCapitol_4A.getDenumire());
        cultura.setCapCultura(capCultura);
        /*nrHA + nrARI*/
        cultura.setSuprafata(SuprafataHelper.transformToMP(
                sectiuneCapitol_4A.getNrARI() != null ? sectiuneCapitol_4A.getNrARI().getValue() : null,
                sectiuneCapitol_4A.getNrHA() != null ? sectiuneCapitol_4A.getNrHA().getValue() : null
        ));
        /*referintaGeoXml*/
        List<GeometrieCultura> listaGeo = new ArrayList<>();
        List<GEOMETRIE_CULTURA> geoInfo = sectiuneCapitol_4A.getGeometrieCultura();
        if (geoInfo != null) {
