 ...
 */
@Service("capitol_15aService")
@Transactional(value = "registruTransactionManager", rollbackFor = RanBusinessException.class)
public class Capitol_15aServiceImpl extends AbstractCapitolFara0XXService {
        ...
        if (gospodarie.getContracts() != null && !gospodarie.getContracts().isEmpty()) {
            ...
            for (Contract contract : gospodarie.getContracts()) {
                ...
                if (xmlRands.contains(rand)) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.SECTIUNE_CAPITOL_DUPLICATA, "contract_arendare", "nrCrt", contract.getNrCrt());
                } else {
                    xmlRands.add(rand);
                }
    ...
    public void addUpdateDateRegistru(RanDocDTO ranDocDTO) {
        Gospodarie oldGospodarie = gospodarieRepository.findByUatAndIdentificator(ranDocDTO.getSirutaUAT(), ranDocDTO.getIdentificatorGospodarie());
        List<Contract> oldContracts = contractRepository.findByTipContractAndGospodarieAndFkNomJudet(TipContract.ARENDARE.getCod(), oldGospodarie.getIdGospodarie(), oldGospodarie.getNomUat().getNomJudet().getId());
    ...
    public void populeazaDateCapitol(GospodarieDTO gospodarieDTO, Integer an, Byte semestru) {
        /*CONTRACT - Arendare*/
        gospodarieDTO.setContracts(contractRepository.findByTipContractAndGospodarieAndFkNomJudet(TipContract.ARENDARE.getCod(), gospodarieDTO.getGospodarie().getIdGospodarie(), gospodarieDTO.getGospodarie().getNomUat().getNomJudet().getId()));
    }

    @Override
    public Class getCapitolClass() {
        return Capitol_15a.class;
    }
}
