 ...
 */
@Service("capitol_8Service")
@Transactional(value = "registruTransactionManager", rollbackFor = RanBusinessException.class)
public class Capitol_8ServiceImpl extends AbstractCapitolCuTotaluriService {
        ...
        if (gospodarie.getCategorieAnimals() != null) {
            ...
            for (CategorieAnimal categorieAnimal : gospodarie.getCategorieAnimals()) {
                ...
                if (xmlCodValues.contains(codValue)) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.SECTIUNE_CAPITOL_DUPLICATA, "categorie_animale_evolutie_efectiva", CapCategorieAnimal.getCodeColumn(), codValue);
                } else {
                    xmlCodValues.add(codValue);
                }
                /*CATEGORIE_ANIMAL - CAP_CATEGORIE_ANIMAL*/
                ro.uti.ran.core.model.registru.CapCategorieAnimal capCategorieAnimal = nomSrv.getNomenclatorForStringParam(CapCategorieAnimal, codValue, dataRaportare, TipCapitol.CAP8.name());
                if (capCategorieAnimal == null) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.NOMENCLATOR_RECORD_NOT_FOUND_AT_DATE, "categorie_animale_evolutie_efectiva", CapCategorieAnimal.getCodeColumn(), codValue, dataRaportare);
                }
                 /*codNomenclator*/
                if (!categorieAnimal.getCapCategorieAnimal().getCod().equals(capCategorieAnimal.getCod())) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.COD_NOMENCLATOR_INVALID, categorieAnimal.getCapCategorieAnimal().getCod(), "categorie_animale_evolutie_efectiva", codValue);
                }
    ...
    public void addUpdateDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        Gospodarie oldGospodarie = gospodarieRepository.findByUatAndIdentificator(ranDocDTO.getSirutaUAT(), ranDocDTO.getIdentificatorGospodarie());
        List<CategorieAnimal> oldCategorieAnimalsDinAn = categorieAnimalRepository.findByAnAndSemestruAndGospodarieAndCapitolAndFkNomJudet(ranDocDTO.getAnRaportare(), ranDocDTO.getSemestruRaportare().byteValue(), oldGospodarie.getIdGospodarie(), TipCapitol.CAP8, oldGospodarie.getNomUat().getNomJudet().getId());
        if (ranDocDTO.getGospodarie().getCategorieAnimals() != null) {
            ...
            regenerareTotaluri(oldGospodarie,
                    ...
                    "categorie_animale_evolutie_efectiva");
        }
    }
     ...
     * @param ranDocDTO info trimse prin xml
     */
    public void anuleazaDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        Gospodarie oldGospodarie = gospodarieRepository.findByUatAndIdentificator(ranDocDTO.getSirutaUAT(), ranDocDTO.getIdentificatorGospodarie());
        List<CategorieAnimal> oldCategorieAnimalsDinAn = categorieAnimalRepository.findByAnAndSemestruAndGospodarieAndCapitolAndFkNomJudet(ranDocDTO.getAnRaportare(), ranDocDTO.getSemestruRaportare().byteValue(), oldGospodarie.getIdGospodarie(), TipCapitol.CAP8, oldGospodarie.getNomUat().getNomJudet().getId());
    ...
    public void populeazaDateCapitol(GospodarieDTO gospodarieDTO, Integer an, Byte semestru) {
        ...
        gospodarieDTO.setCategorieAnimals(categorieAnimalRepository.findByAnAndSemestruAndGospodarieAndCapitolAndFkNomJudet(an, semestru, gospodarieDTO.getGospodarie().getIdGospodarie(), TipCapitol.CAP8, gospodarieDTO.getGospodarie().getNomUat().getNomJudet().getId()));
    }

    @Override
    public Class getCapitolClass() {
        return Capitol_8.class;
    }
}
