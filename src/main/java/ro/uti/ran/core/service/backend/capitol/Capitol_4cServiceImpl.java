...
import static ro.uti.ran.core.service.backend.utils.TipCapitol.CAP4c;

/**
 * Suprafața cultivată cu legume si cartofi in grădinile familiale pe raza localității
 * Created by Dan on 28-Oct-15.
 */
@Service("capitol_4cService")
@Transactional(value = "registruTransactionManager", rollbackFor = RanBusinessException.class)
public class Capitol_4cServiceImpl extends AbstractCapitolCuTotaluriService {

    private static final Logger logger = LoggerFactory.getLogger(Capitol_4cServiceImpl.class);
        ...
        if (gospodarie.getCulturas() != null) {
            ...
            for (Cultura cultura : gospodarie.getCulturas()) {
                ...
                if (xmlCodValues.contains(codValue)) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.SECTIUNE_CAPITOL_DUPLICATA, "cultura_in_gradini", CapCultura.getCodeColumn(), codValue);
                } else {
                    xmlCodValues.add(codValue);
                }
                /*CULTURA - NOM_CULTURA*/
                ro.uti.ran.core.model.registru.CapCultura nomCultura = nomSrv.findCapByCodAndCodRandAndCapitolAndDataValabilitate(CapCultura, cultura.getCapCultura().getCod(), cultura.getCapCultura().getCodRand(), CAP4c.name(), dataRaportare);
                if (nomCultura == null) {
                ...
                /*codNomenclator*/
                if (!cultura.getCapCultura().getCod().equals(nomCultura.getCod())) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.COD_NOMENCLATOR_INVALID, cultura.getCapCultura().getCod(), "cultura_in_gradini", codValue);
                }
                //
                cultura.setCapCultura(nomCultura);
                ...
                if (cultura.getGeometrieCulturi() != null && !cultura.getGeometrieCulturi().isEmpty()) {
                    if (RanConstants.NOM_IS_FORMULA_DA.equals(cultura.getCapCultura().getIsFormula())) {
                        throw new DateRegistruValidationException(DateRegistruValidationCodes.GEOMETRIE_CULTURA_RAND_TOTAL, "cultura_in_gradini", cultura.getCapCultura().getCod(), cultura.getCapCultura().getCodRand());
                    }
                }
                /*fkNomJudet*/
    ...
    public void addUpdateDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        Gospodarie oldGospodarie = gospodarieRepository.findByUatAndIdentificator(ranDocDTO.getSirutaUAT(), ranDocDTO.getIdentificatorGospodarie());
        List<Cultura> oldCulturasDinAn = culturaRepository.findByAnAndGospodarieAndCapitolAndFkNomJudet(ranDocDTO.getAnRaportare(), oldGospodarie.getIdGospodarie(), CAP4c, oldGospodarie.getNomUat().getNomJudet().getId());
    ...
    protected Object getOldRandTotal(Integer an, Integer semestru, Long idGospodarie, Long idNomParent) {
        return culturaRepository.findByAnAndGospodarieAndCapitolAndNomCultura(an, idGospodarie, CAP4c, idNomParent);
    }
    ...
    public void populeazaDateCapitol(GospodarieDTO gospodarieDTO, Integer an, Byte semestru) {
        /*CULTURA - din an*/
        gospodarieDTO.setCulturas(culturaRepository.findByAnAndGospodarieAndCapitolAndFkNomJudet(an, gospodarieDTO.getGospodarie().getIdGospodarie(), CAP4c, gospodarieDTO.getGospodarie().getNomUat().getNomJudet().getId()));
