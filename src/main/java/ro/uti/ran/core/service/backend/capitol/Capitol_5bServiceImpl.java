...
/**
 * Suprafața plantațiilor pomicole si numărul pomilor pe raza localității
 * Created by Dan on 29-Oct-15.
 */
@Service("capitol_5bService")
@Transactional(value = "registruTransactionManager", rollbackFor = RanBusinessException.class)
public class Capitol_5bServiceImpl extends AbstractCapitolCuTotaluriService {
        ...
        if (gospodarie.getPlantaties() != null) {
            ...
            for (Plantatie plantatie : gospodarie.getPlantaties()) {
                ...
                if (xmlCodValues.contains(codValue)) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.SECTIUNE_CAPITOL_DUPLICATA, "pom_plantatii_pomicole", CapPlantatie.getCodeColumn(), codValue);
                } else {
                    xmlCodValues.add(codValue);
                }
                /*PLANTATIE - CAP_PLANTATIE*/
                ro.uti.ran.core.model.registru.CapPlantatie capPlantatie = nomSrv.getNomenclatorForStringParam(CapPlantatie, codValue, dataRaportare, TipCapitol.CAP5b.name());
                if (capPlantatie == null) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.NOMENCLATOR_RECORD_NOT_FOUND_AT_DATE, "pom_plantatii_pomicole", CapPlantatie.getCodeColumn(), codValue, dataRaportare);
                }
                 /*codNomenclator*/
                if (!plantatie.getCapPlantatie().getCod().equals(capPlantatie.getCod())) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.COD_NOMENCLATOR_INVALID, plantatie.getCapPlantatie().getCod(), "pom_plantatii_pomicole", codValue);
                }
    ...
    public void addUpdateDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        Gospodarie oldGospodarie = gospodarieRepository.findByUatAndIdentificator(ranDocDTO.getSirutaUAT(), ranDocDTO.getIdentificatorGospodarie());
        List<Plantatie> oldPlantatieDinAn = plantatieRepository.findByAnAndGospodarieAndCapitolAndFkNomJudet(ranDocDTO.getAnRaportare(), oldGospodarie.getIdGospodarie(), TipCapitol.CAP5b, oldGospodarie.getNomUat().getNomJudet().getId());
        if (ranDocDTO.getGospodarie().getPlantaties() != null) {
            ...
            regenerareTotaluri(oldGospodarie,
                    ...
                    CapPlantatie,
                    "Plantatie",
                    new String[]{"suprafata", "nrPomRod"},
                    "capPlantatie",
                    "capPlantatie",
                    new String[]{"nrARI/nrHA", "nrPomiPeRod"},
                    new String[]{RanConstants.UM_MP, RanConstants.UM_NR_POMI},
                    "pom_plantatii_pomicole");
        }
    }
    ...
    protected Integer[] getDbTotal(Integer an, Integer semestru, Long idGospodarie, Long idNomParent) {
        ...
        if (oldRandTotal != null) {
            return new Integer[]{oldRandTotal.getSuprafata(), oldRandTotal.getNrPomRod()};
        }
        return null;
    }
    ...
    public void populeazaDateCapitol(GospodarieDTO gospodarieDTO, Integer an, Byte semestru) {
        /*PLANTATIE - din an*/
        gospodarieDTO.setPlantaties(plantatieRepository.findByAnAndGospodarieAndCapitolAndFkNomJudet(an, gospodarieDTO.getGospodarie().getIdGospodarie(), TipCapitol.CAP5b, gospodarieDTO.getGospodarie().getNomUat().getNomJudet().getId()));
