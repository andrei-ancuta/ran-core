...
/**
 * Vii, pepiniere viticole si hameiști situate pe raza localității
 * Created by Dan on 29-Oct-15.
 */
@Service("capitol_5dService")
@Transactional(value = "registruTransactionManager", rollbackFor = RanBusinessException.class)
public class Capitol_5dServiceImpl extends AbstractCapitolCuTotaluriService {
...
/*gospodaria trebuie sa existe in baza*/
        ...
        if (gospodarie.getPlantaties() != null) {
            ...
            for (Plantatie plantatie : gospodarie.getPlantaties()) {
                ...
                if (xmlCodValues.contains(codValue)) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.SECTIUNE_CAPITOL_DUPLICATA, "vii_hamei", CapPlantatie.getCodeColumn(), codValue);
                } else {
                    xmlCodValues.add(codValue);
                }
                /*PLANTATIE - CAP_PLANTATIE*/
                ro.uti.ran.core.model.registru.CapPlantatie capPlantatie = nomSrv.getNomenclatorForStringParam(CapPlantatie, codValue, dataRaportare, TipCapitol.CAP5d.name());
                if (capPlantatie == null) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.NOMENCLATOR_RECORD_NOT_FOUND_AT_DATE, "vii_hamei", CapPlantatie.getCodeColumn(), codValue, dataRaportare);
                }
                /*codNomenclator*/
                if (!plantatie.getCapPlantatie().getCod().equals(capPlantatie.getCod())) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.COD_NOMENCLATOR_INVALID, plantatie.getCapPlantatie().getCod(), "vii_hamei", codValue);
                }
    ...
    public void addUpdateDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        Gospodarie oldGospodarie = gospodarieRepository.findByUatAndIdentificator(ranDocDTO.getSirutaUAT(), ranDocDTO.getIdentificatorGospodarie());
        List<Plantatie> oldPlantatieDinAn = plantatieRepository.findByAnAndGospodarieAndCapitolAndFkNomJudet(ranDocDTO.getAnRaportare(), oldGospodarie.getIdGospodarie(), TipCapitol.CAP5d, oldGospodarie.getNomUat().getNomJudet().getId());
    ...
    public void populeazaDateCapitol(GospodarieDTO gospodarieDTO, Integer an, Byte semestru) {
        /*PLANTATIE - din an*/
        gospodarieDTO.setPlantaties(plantatieRepository.findByAnAndGospodarieAndCapitolAndFkNomJudet(an, gospodarieDTO.getGospodarie().getIdGospodarie(), TipCapitol.CAP5d, gospodarieDTO.getGospodarie().getNomUat().getNomJudet().getId()));
