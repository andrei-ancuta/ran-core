...
import static ro.uti.ran.core.service.backend.utils.TipCapitol.CAP4b2;

/**
 * Suprafața cultivată in solarii si alte spații protejate pe raza localității
 * Created by Dan on 28-Oct-15.
 */
@Service("capitol_4b2Service")
@Transactional(value = "registruTransactionManager", rollbackFor = RanBusinessException.class)
public class Capitol_4b2ServiceImpl extends AbstractCapitolCuTotaluriService {

    private static final Logger logger = LoggerFactory.getLogger(Capitol_4b2ServiceImpl.class);
        ...
        if (gospodarie.getCulturas() != null) {
            ...
            for (Cultura cultura : gospodarie.getCulturas()) {
                ...
                if (xmlCodValues.contains(codValue)) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.SECTIUNE_CAPITOL_DUPLICATA, "cultura_in_spatiu_protejat", CapCultura.getCodeColumn(), codValue);
                } else {
                    xmlCodValues.add(codValue);
                }
                /*CULTURA - CAP_CULTURA*/
                ro.uti.ran.core.model.registru.CapCultura capCultura = nomSrv.findCapByCodAndCodRandAndCapitolAndDataValabilitate(CapCultura, cultura.getCapCultura().getCod(), cultura.getCapCultura().getCodRand(), CAP4b2.name(), dataRaportare);
                if (capCultura == null) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.NOMENCLATOR_RECORD_NOT_FOUND_AT_DATE, "cultura_in_spatiu_protejat", CapCultura.getCodeColumn(), codValue, dataRaportare);
                }
                /*codNomenclator*/
                if (!cultura.getCapCultura().getCod().equals(capCultura.getCod())) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.COD_NOMENCLATOR_INVALID, cultura.getCapCultura().getCod(), "cultura_in_spatiu_protejat", codValue);
                }
                ...
                if (cultura.getGeometrieCulturi() != null && !cultura.getGeometrieCulturi().isEmpty()) {
                    if (RanConstants.NOM_IS_FORMULA_DA.equals(cultura.getCapCultura().getIsFormula())) {
                        throw new DateRegistruValidationException(DateRegistruValidationCodes.GEOMETRIE_CULTURA_RAND_TOTAL, "cultura_in_spatiu_protejat", cultura.getCapCultura().getCod(), cultura.getCapCultura().getCodRand());
                    }
                }
                /*fkNomJudet*/
    ...
    public void addUpdateDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        Gospodarie oldGospodarie = gospodarieRepository.findByUatAndIdentificator(ranDocDTO.getSirutaUAT(), ranDocDTO.getIdentificatorGospodarie());
        List<Cultura> oldCulturasDinAn = culturaRepository.findByAnAndGospodarieAndTipSpatiuProtAndCapitolAndFkNomJudet(ranDocDTO.getAnRaportare(), oldGospodarie.getIdGospodarie(), RanConstants.NOM_TIP_SPATIU_PROT_COD_SOLAR, CAP4b2, oldGospodarie.getNomUat().getNomJudet().getId());
    ...
    protected Object getOldRandTotal(Integer an, Integer semestru, Long idGospodarie, Long idNomParent) {
        return culturaRepository.findByAnAndGospodarieAndCapitolAndNomCultura(an, idGospodarie, CAP4b2, idNomParent);
    }
    ...
    public void populeazaDateCapitol(GospodarieDTO gospodarieDTO, Integer an, Byte semestru) {
        ...
        gospodarieDTO.setCulturas(culturaRepository.findByAnAndGospodarieAndTipSpatiuProtAndCapitolAndFkNomJudet(an, gospodarieDTO.getGospodarie().getIdGospodarie(), RanConstants.NOM_TIP_SPATIU_PROT_COD_SOLAR, CAP4b2, gospodarieDTO.getGospodarie().getNomUat().getNomJudet().getId()));
