...
@Service("capitol_12b2Service")
@Transactional(value = "registruTransactionManager")
public class Capitol_12b2ServiceImpl extends AbstractCapitolCentralizatorCuTotaluriService {

    @Override
    public void populeazaDateCapitol(CapitolCentralizatorDTO capitolCentralizatorDTO) throws RanBusinessException {
        NomCapitol nomCapitol = nomSrv.getNomenclatorForStringParam(NomenclatorCodeType.NomCapitol, TipCapitol.CAP12b2.name(), new DataRaportareValabilitate());
        if (nomCapitol == null) {
            throw new InterogareDateRegistruException(InterogareDateRegistruCodes.CAPITOL_CENTRALIZATOR_NOT_FOUND, TipCapitol.CAP12b2.name());
        }
        ...
        List<CulturaProd> culturaSpatiuProtProdList = culturaProdRepository.findAllByTipSpatiuAndYearForSirutaAndFkNomJudet(capitolCentralizatorDTO.getAn(), TipSpatiuProtejat.SO.name(), capitolCentralizatorDTO.getCodSiruta(), TipCapitol.CAP12b2, nomUat.getNomJudet().getId());
        if (culturaSpatiuProtProdList != null && !culturaSpatiuProtProdList.isEmpty()) {
            List<RandCapitol_12b2DTO> randuri = new ArrayList<RandCapitol_12b2DTO>();
            for (CulturaProd culturaSpatiuProd : culturaSpatiuProtProdList) {

                RandCapitol_12b2DTO rand = new RandCapitol_12b2DTO();
                rand.setSuprafata(culturaSpatiuProd.getSuprafata());
                rand.setProductieMedie(NOM_VALOARE_DA.equals(culturaSpatiuProd.getCapCulturaProd().getIsProdMedie()) ? culturaSpatiuProd.getProdMedie() : null);
                rand.setProductieTotala(culturaSpatiuProd.getProdTotal());
                rand.setDenumire(culturaSpatiuProd.getCapCulturaProd().getDenumire());
                rand.setCod(culturaSpatiuProd.getCapCulturaProd().getCod());
                rand.setCodRand(culturaSpatiuProd.getCapCulturaProd().getCodRand());
                randuri.add(rand);
            }
            capitolCentralizatorDTO.setRandCapitolList(randuri);
        } else {
            //Capitolul este gol, arunc exceptie
            throw new InterogareDateRegistruException(InterogareDateRegistruCodes.CAPITOL_CENTRALIZATOR_EMPTY, capitolCentralizatorDTO.getAn(), TipCapitol.CAP12b2.name(), capitolCentralizatorDTO.getCodSiruta());
        }
    }

    @Override
    public Class getCapitolClass() {
        return Capitol_12b2.class;
    }
    ...
    public void valideazaDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        valideazaDateRegistru(ranDocDTO, TipCapitol.CAP12b2, NomenclatorCodeType.CapCulturaProd, new Tuple[]{
    ...
    protected void addUpdateDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        ...
        List<RandCapitol_12b2DTO> randuriCapitolCentralizator = (List<RandCapitol_12b2DTO>) ranDocDTO.getRanduriCapitolCentralizator();
        NomUat nomUat = nomUatRepository.findByCodSiruta(ranDocDTO.getSirutaUAT());
        Set<Long> childrenIds = new HashSet<Long>();

        for (RandCapitol_12b2DTO rand : randuriCapitolCentralizator) {
            CapCulturaProd capCulturaProd = nomSrv.getNomenlatorForId(NomenclatorCodeType.CapCulturaProd, rand.getNomId());
            NomTipSpatiuProt nomTipSpatiuProt = nomSrv.getNomenclatorForStringParam(NomenclatorCodeType.NomTipSpatiuProt, TipSpatiuProtejat.SO.name(), new DataRaportareValabilitate());
    ...
    protected void anuleazaDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        ...
        NomUat nomUat = nomUatRepository.findByCodSiruta(ranDocDTO.getSirutaUAT());
        List<CulturaProd> culturaSpatiuProtProdList = culturaProdRepository.findAllByTipSpatiuAndYearForSirutaAndFkNomJudet(ranDocDTO.getAnRaportare(), TipSpatiuProtejat.SO.name(), ranDocDTO.getSirutaUAT(), TipCapitol.CAP12b2, nomUat.getNomJudet().getId());
        ...
        //Sterg tot capitolul
        for (CulturaProd oldCulturaProd : culturaSpatiuProtProdList) {
