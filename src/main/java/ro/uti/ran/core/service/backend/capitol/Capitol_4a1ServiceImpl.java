...
import static ro.uti.ran.core.service.backend.utils.TipCapitol.CAP4a1;

/**
 * Culturi succesive în câmp, culturi intercalate, culturi modificate genetic pe raza localității
 * Created by Dan on 28-Oct-15.
 */
@Service("capitol_4a1Service")
@Transactional(value = "registruTransactionManager", rollbackFor = RanBusinessException.class)
public class Capitol_4a1ServiceImpl extends AbstractCapitolCuTotaluriService {

    private static final Logger logger = LoggerFactory.getLogger(Capitol_4a1ServiceImpl.class);
        ...
        if (gospodarie.getCulturas() != null) {
            ...
            for (Cultura cultura : gospodarie.getCulturas()) {
                ...
                if (xmlCodValues.contains(codValue)) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.SECTIUNE_CAPITOL_DUPLICATA, "cultura_speciala_in_camp", CapCultura.getCodeColumn(), codValue);
                } else {
                    xmlCodValues.add(codValue);
                }
                /*CULTURA - NOM_CULTURA*/
                CapCultura capCultura = nomSrv.findCapByCodAndCodRandAndCapitolAndDataValabilitate(CapCultura, cultura.getCapCultura().getCod(), cultura.getCapCultura().getCodRand(), CAP4a1.name(), dataRaportare);
                if (capCultura == null) {
                ...
                if (!cultura.getCapCultura().getCod().equals(capCultura.getCod())) {
                    throw new DateRegistruValidationException(DateRegistruValidationCodes.COD_NOMENCLATOR_INVALID, cultura.getCapCultura().getCod(), "cultura_speciala_in_camp", codValue);
                }
                //
                ...
                if (cultura.getGeometrieCulturi() != null && !cultura.getGeometrieCulturi().isEmpty()) {
                    if (RanConstants.NOM_IS_FORMULA_DA.equals(cultura.getCapCultura().getIsFormula())) {
                        throw new DateRegistruValidationException(DateRegistruValidationCodes.GEOMETRIE_CULTURA_RAND_TOTAL, "cultura_speciala_in_camp", cultura.getCapCultura().getCod(), cultura.getCapCultura().getCodRand());
                    }
                }
                /*fkNomJudet*/
    ...
    public void addUpdateDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        Gospodarie oldGospodarie = gospodarieRepository.findByUatAndIdentificator(ranDocDTO.getSirutaUAT(), ranDocDTO.getIdentificatorGospodarie());
        List<Cultura> oldCulturasDinAn = culturaRepository.findByAnAndGospodarieAndCapitolAndFkNomJudet(ranDocDTO.getAnRaportare(), oldGospodarie.getIdGospodarie(), CAP4a1, oldGospodarie.getNomUat().getNomJudet().getId());
    ...
    protected Object getOldRandTotal(Integer an, Integer semestru, Long idGospodarie, Long idNomParent) {
        return culturaRepository.findByAnAndGospodarieAndCapitolAndNomCultura(an, idGospodarie, CAP4a1, idNomParent);
    }
    ...
    public void populeazaDateCapitol(GospodarieDTO gospodarieDTO, Integer an, Byte semestru) {
        /*CULTURA - din an*/
        gospodarieDTO.setCulturas(culturaRepository.findByAnAndGospodarieAndCapitolAndFkNomJudet(an, gospodarieDTO.getGospodarie().getIdGospodarie(), CAP4a1, gospodarieDTO.getGospodarie().getNomUat().getNomJudet().getId()));
