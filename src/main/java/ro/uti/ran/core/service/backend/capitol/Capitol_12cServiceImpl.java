...
@Service("capitol_12cService")
@Transactional(value = "registruTransactionManager")
public class Capitol_12cServiceImpl extends AbstractCapitolCentralizatorCuTotaluriService {

    @Override
    public void populeazaDateCapitol(CapitolCentralizatorDTO capitolCentralizatorDTO) throws RanBusinessException {
        NomCapitol nomCapitol = nomSrv.getNomenclatorForStringParam(NomenclatorCodeType.NomCapitol, TipCapitol.CAP12c.name(), new DataRaportareValabilitate());
        if (nomCapitol == null) {
            throw new InterogareDateRegistruException(InterogareDateRegistruCodes.CAPITOL_CENTRALIZATOR_NOT_FOUND, TipCapitol.CAP12c.name());
        }
        ...
        if (culturaProds != null && !culturaProds.isEmpty()) {
            List<RandCapitol_12cDTO> randuri = new ArrayList<RandCapitol_12cDTO>();
            for (CulturaProd cp : culturaProds) {

                RandCapitol_12cDTO rand = new RandCapitol_12cDTO();
            ...
            throw new InterogareDateRegistruException(InterogareDateRegistruCodes.CAPITOL_CENTRALIZATOR_EMPTY, capitolCentralizatorDTO.getAn(), TipCapitol.CAP12c.name(), capitolCentralizatorDTO.getCodSiruta());
        }
    }

    @Override
    public Class getCapitolClass() {
        return Capitol_12c.class;
    }
    ...
    public void valideazaDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        valideazaDateRegistru(ranDocDTO, TipCapitol.CAP12c, NomenclatorCodeType.CapCulturaProd, new Tuple[]{
    ...
    protected void addUpdateDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        ...
        List<RandCapitol_12cDTO> randuriCapitolCentralizator = (List<RandCapitol_12cDTO>) ranDocDTO.getRanduriCapitolCentralizator();
        NomUat nomUat = nomUatRepository.findByCodSiruta(ranDocDTO.getSirutaUAT());
        Set<Long> childrenIds = new HashSet<Long>();

        for (RandCapitol_12cDTO rand : randuriCapitolCentralizator) {
    ...
    protected void anuleazaDateRegistru(RanDocDTO ranDocDTO) throws DateRegistruValidationException {
        ...
        NomUat nomUat = nomUatRepository.findByCodSiruta(ranDocDTO.getSirutaUAT());
        List<CulturaProd> culturaProdList = culturaProdRepository.findAllByTipCapitolAndYearForSirutaAndFkNomJudet(ranDocDTO.getAnRaportare(), TipCapitol.CAP12c, ranDocDTO.getSirutaUAT(), nomUat.getNomJudet().getId());
